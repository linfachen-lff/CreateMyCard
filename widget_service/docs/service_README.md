...
- Data capabilities, event capabilities, and assets are selected by App/ROM left-closed,
  right-open ranges in `cloud/shared/data/capabilities/registry_ranges.json`.
...
- With model mock disabled, all three generation routes use `A2UIModelClient.generate()` and the internal
  ...
  `cloud/shared/custom/llmclient.py` as fallback by default. Configure them with `WIDGET_SERVICE_OPENAI_MASTER_CLIENT` and
...
- DeepSeek Platform reads its SK only from the STS key configured by
  `WIDGET_SERVICE_DEEPSEEK_PLATFORM_SECRET_KEY_STS_CONFIG_KEY`. The key name and secret reader are supplied by the
  current zone Adapter and have no shared default. Its remaining static request fields use the
...
- The llmclient WebSocket request is configured by the `WIDGET_SERVICE_DEEPSEEK_*` fields declared in
  `.env.example`, covering credentials, endpoint, model/user/request identifiers, sampling, maximum tokens,
  thinking/usage flags, and receive timeout. Zone-sensitive values have blank shared defaults and are supplied by
  the current zone Adapter.
...
- `WIDGET_SERVICE_ENABLE_ARTIFACT_DOWNLOAD_MOCK=true` by default. Multi-round source artifacts are read only from `cloud/zone/runtime/workspace/mock_obs`; missing mock files do not fall back to the network. Set it to `false` to download from the validated HTTPS artifact URL.
...
`cloud/shared` 是蓝区到绿区的完整复制单元，区内配置、日志和生成产物保留在 `cloud/zone`。
`cloud/shared/app/application.py` 只负责创建应用，不是独立启动入口；服务必须通过外层启动脚本加载。
...
```powershell
cd widget_service
.\.venv\Scripts\python tests\test_running_ws_server.py
```

该脚本会调用真实 `generateWidgetCard`，读取服务保存的 artifact，通过
`cloud/shared/services/card_validation/` Python API 再校验一次并打印诊断。当前 mock 输出包含
...
cd widget_service
# 运行全部单功能用例
.\.venv\Scripts\python -m pytest tests\test_running_ws_features.py -s -q
# 只测能力概述
.\.venv\Scripts\python -m pytest tests\test_running_ws_features.py::test_live_widget_capability_overview -s -q
# 只测一个数据能力 schema
.\.venv\Scripts\python -m pytest "tests\test_running_ws_features.py::test_live_each_data_capability_schema[ViewWeather]" -s -q
# 只测 A2UI Form 或 Compact DSL 生成
.\.venv\Scripts\python -m pytest tests\test_running_ws_features.py::test_live_generate_widget_card -s -q
.\.venv\Scripts\python -m pytest tests\test_running_ws_features.py::test_live_generate_widget_card_compact_dsl -s -q
```
...
```powershell
cd widget_service
.\.venv\Scripts\python tests\test_running_ws_multi_round.py
# 或显示每轮响应：
.\.venv\Scripts\python -m pytest tests\test_running_ws_multi_round.py -s -q
```
...
```powershell
cd widget_service
.\.venv\Scripts\python -m pytest tests\test_service_units.py -s -q
```

真实 WebSocket 联调时，业务日志由 `cloud.start_websocket_server` 进程输出，应在服务终端查看；
本地文件日志位于 `cloud/zone/runtime/logs/agent_YYYYMMDD.log`。客户端测试终端只显示请求响应和脚本打印的校验报告。
