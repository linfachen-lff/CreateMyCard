...
- 绑定/DataModel：所有可见表达式引用、原生绑定路径和 `formatString` 路径都能在 `updateDataModel.value` 找到；数据能力运行时字段至少初始化到可推导根结构；模板相对路径除外。
...
- 动态展示值优先完整表达式 `{{ ... }}`；`{"path":"/..."}` 和 `formatString` 只在表达式不适合、模板相对路径或端侧要求原生绑定时兜底。
